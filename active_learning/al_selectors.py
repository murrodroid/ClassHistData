...
def _diverse_maxmin(C, k):
    if issparse(C):
        C = C.tocsr()
        norms = np.sqrt(C.power(2).sum(axis=1)).A1
        norms[norms == 0] = 1.0
        Cn = C.multiply(1.0 / norms[:, None]).tocsr()
        n = Cn.shape[0]
        sel = [0]
        sim = (Cn @ Cn.getrow(0).T).toarray().ravel()
        for _ in range(1, min(k, n)):
            dist = 1.0 - sim
            j = int(dist.argmax())
            sel.append(j)
            sim = np.maximum(sim, (Cn @ Cn.getrow(j).T).toarray().ravel())
        return sel
    C = np.asarray(C)
...
def select_active(data_idx, committee_models, k, batch_size=2048, diversity_factor=10):
    ...
    ask_rel, _ = committee.query(pool, n_instances=m)
    rel_idx = np.atleast_1d(np.asarray(ask_rel)).astype(int).tolist()
    ...
    rel_idx_arr = np.asarray(rel_idx, dtype=int)
    return ul[rel_idx_arr[pick_rel]].astype(int).tolist()